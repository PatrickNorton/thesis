...
probability. However, if $L$ could accept even in the face of an arbitrarily
malicious verifier, it would mean that $L$ would have some means of computing
the problem on its own; hence we would only ever be able to accept languages in
$\BPP$ (since $L$ is a $\BPP$ machine).
...
% TODO: BGKW88 says that any MIP has a variant that is complete (if x \in L then V
% always accepts)
...
% TODO: Rewrite this to be more in line with the other definitions
...
% FIXME: Super-perfect zero-knowledge proofs now exist
...
% TODO: Define view (somewhere common to all the things that care about views)
\begin{defn}\label{def:possible-commit}\index{possible commitment}
  Let $\sigma \in \{0, 1\}$. A reciever's view of an interaction $(r, m)$ is an
  \emph{possible $\sigma$-commmitment} if there exists a string $s$ such that $m$
  describes the messages received by $R$ when $R$ uses local coins $r$ and
  interacts with machine $S$ that uses local coins $s$ and has input
  $(\sigma, 1^{n})$.
\end{defn}
...
% TODO: More explanation & potentially an example?

\begin{defn}\label{def:alg-comm-scheme}\index{commitment scheme!algebraic}
  An \emph{algebraic commitment scheme} is % TODO
\end{defn}
...
% TODO: Why do we parameterize over the verifier here?
...
\begin{defn}\label{def:alg-circuit}\index{satisfying assignment!algebraic circuit}
...
\begin{figure} % TODO: Make clear nodes' original operation
  \begin{center}
    \begin{tikzpicture}[>=stealth,every node/.style={minimum size=0.75cm},scale=2]
...
\section{An upper bound on $\PCP$ queries}\label{sec:pcp-query-complexity}
...
\begin{algorithm}[htbp]
  ...
  $V'$: choose $S \subseteq \mathbb{F}$ with $\abs{S} < q$\; % FIXME: Is this random or predetermined?
  $V'$: choose random $t \in \mathbb{F} \setminus S$\;
  % FIXME: What is meant by curve (polynomial?)
  % NOTE: A is "query set" of V on input S
  $V'$: compute curve $\gamma: \mathbb{F} \rightarrow \mathbb{F}^{m}$ of degree $q$ such that
  $\{\gamma(s)\}_{s \in S} = A$ and $\gamma(t) = r$\;
  $V'$: Send $\gamma$ to $P'$\;
  $P'$: Reply with the coefficients of the polynomial
  $\rho: \mathbb{F} \rightarrow \mathbb{F}$ with $\deg(\rho) < mdq$ that is the restriction of
  $R$ to $\gamma$\;
  $V$: query $R$ at $r$ and recieve answer $a$\; % FIXME: Should this be V'?
  \If{$a \ne \rho(t)$}{\nllabel{line:check-rho}
    reject\;
  }
  % FIXME: TypeError: Should be \rho(A)?
  $V'$: Simulate $V(x)$, answering all queries $a \in A$ with $\gamma(A)$\;
...
\begin{proof}
  ...
  To show $V'$ runs in polynomial time, % TODO: Complexity of the random choice
  % TODO
  ...
  $dq$, then it can agree with $R\gamma$ at most $mdq$ distinct points without being
  equal. Hence, since we query it at a random point in $\mathbb{F} \setminus S$, if the
  check in line~\ref{line:check-rho} passes with at probability bigger than
  $\frac{mdq}{\abs{\mathbb{F}} - q}$, it must mean that $\rho = R\gamma$ on more than
  $mdq$ points. Thus, $\rho = R\gamma$ and so we will get the correct answer for all of
  our queries on $A$. If it passes with a lower probability, then we are within
  our soundness error overall and so the result does not matter.
\end{proof}
...
\section{$\PCPP$ for polynomial summation}\label{sec:pcpp-poly-sum}
