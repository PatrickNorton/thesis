...
immediately apparent here why $\delta$ has the property it does.

The form of $\delta_{x}$ defined in \cref{eqn:delta-poly-small} has further use to us
than just being simpler.
...
oracle, it can still do more operations with an oracle. Further, for the
question of $\P$ vs.\ $\NP$ that we will discuss in \cref{sec:rel-p-np}, this
means that the question we care about is whether $\NP \subseteq^{?} \P$ relativizes. As
such, the question we are asking simplifies to determining where
$\P^{A} = \NP^{A}$ and where $\P^{A} \subsetneq \NP^{A}$.
...
\begin{proof}
  ...
  is itself $\PSPACE$, hence \cref{alg:l-tilde-det} is a $\PSPACE$
  algorithm.

  Next, we show that \cref{alg:l-tilde-det} determines $\tilde{L}$.
  First, from \cref{cor:low-degree-boolean}, we know that for any $n$,
  \begin{equation}
