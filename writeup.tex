...
\begin{lemma}[Schwartz-Zippel lemma; \cite{Sch80,Zip79}]%
  \label{lem:schwartz-zippel}\index{Schwartz-Zippel lemma}
...
\begin{defn}\label{def:hybrid-sim}\index{hybrid simulator}
...
Finally, it is time to introduce a verifier for $\Sum$. \TODO{More}
...
\begin{algorithm}[htbp]
  ...
  Let $S_{0}$ be a simulated copy of \cref{alg:zk-sumcheck-fast} on input
  $(\mathbb{F}, m_{1} + 3m_{2}, \deg(f), H, 0)$\;
  \For{$i \in \{1, 2, 3\}$}{
    \FIXME{Should be weak-ZK simulator (inline how?)}\;
    Let $S_{i}$ be a simulated copy of \cref{alg:zk-sumcheck-fast} on input
    $(\mathbb{F}, k, 2\abs{H}, \abs{H}, \cdot)$\;
  }
  Simulate $\tilde{V}$ to recieve $x, y \in \mathbb{F}^{r + 3s}$\;
  \TODO{Simulate \cref{alg:zk-sumcheck}}\;
  \For{$i \in \{1, 2, 3\}$}{
...
\begin{proof}
  \Cref{lem:g_a-vs-h_c} tells that \cref{item:exists-a,item:exists-c}
...
\begin{algorithm}[htbp] % TODO: If this gets longer, may need to split into proof/verifier
  ...
  \Proof{
    ...
    $b \in H^{m_{2}}$\;\label{line:sample-c}
    \For{$\tau \in \mathbb{F}^{m_{1}+3m_{2}+3}$}{
      ...
      \begin{algomathdisplay}
        \sum_{\substack{z \in H^{m_{1}} \\ b_{1}, b_{2}, b_{3} \in H^{m_{2}} \\
          a \in \{0, 1\}^{3} \\ c_{1}, c_{2}, c_{3} \in H^{k}}}
        [(z, b, a) = \tau]h_{\hat{C}}(\tau, c_{1}, c_{2}, c_{3}) = 0
      \end{algomathdisplay}
    }
    \KwRet{$(\pi_{C}, (\pi_{\tau})_{\tau \in \mathbb{F}^{m_{1}+3m_{2}+3}})$}\;
  }
  \Verifier{
    \For{\TODO{``sufficiently many times so that each test accounts for at least
        1/3 of the verifier’s view.''}}{
      ...
      Simulate \cref{alg:sum-pzk-pcpp} with proof $\pi_{\tau}$\;\nllabel{line:sumcheck-pi-tau} % FIXME: State yielded claim
      \For{$i \in \{1, 2, 3\}$}{
        Query $\hat{C}$ at $(\nu_{i}, \eta_{i})$\;
      }
...
\chapter{Proofs of expected-distance lemmata}\label[appendix]{app:exp-dist-proofs}

\begin{lemma}[{\cref{lem:sum-ev}}]
  Let $\tilde{F}\colon \mathbb{F}^{m} \rightarrow \mathbb{F}$ be $\delta_{\Sigma}$-far from
  $\Sum[\mathbb{F}, m, d, H, \gamma]$, but $\delta_{\RM}$-close to
  $\RM[\mathbb{F}, m, d]$ for some
  $\frac{md}{\abs{\mathbb{F}}} < \delta_{\RM} \le \delta_{\Sigma}$ and $d \ge \abs{H} + 1$. Then,
...
\begin{lemma}[{\cref{lem:sum-ev-close}}]
  Let $\tilde{F}\colon \mathbb{F}^{m} \rightarrow \mathbb{F}$ be $\delta_{\Sigma}$-far from
  $\Sum[\mathbb{F}, m, d, H, \gamma]$, but $\delta_{\RM}$-close to $\RM[\mathbb{F}, m, d]$
  for $\delta_{\Sigma} > \delta_{\RM} \ge \frac{md}{\abs{\mathbb{F}}}$, $\delta_{\RM} < 1/5$, and
  $d \ge \abs{H} + 1$. Then, for all proofs $(\pi_{\Sigma}^{*}, \pi_{P}^{*})$,
